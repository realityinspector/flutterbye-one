...
import { Call, CallOutcome } from '../hooks/useCalls';
...
const CallItem = ({ 
  ...
  const { data: leadData } = useLeads().getLead(call.userLeadId);
    ...
    <Pressable onPress={onPress}>
      ...
      >
        <VStack space={compact ? 1 : 2}>
          ...
          {!compact && !leadName && leadData && (
            <Text fontSize="md" fontWeight="semibold">
              {leadData.globalLead?.contactName || 'Unknown'} - {leadData.globalLead?.companyName || 'Unknown Company'}
            </Text>
          )}
          ...
          {call.outcome && (
            <HStack space={2} alignItems="center">
              <Badge 
                colorScheme={OUTCOME_COLORS[call.outcome as CallOutcome]} 
                variant="subtle" 
                rounded="sm"
              >
                <HStack space={1} alignItems="center">
                  <Icon 
                    as={Feather} 
                    name={OUTCOME_ICONS[call.outcome as CallOutcome]} 
                    size="2xs" 
                    color={`${OUTCOME_COLORS[call.outcome as CallOutcome]}.700`} 
                  />
                  <Text fontSize="2xs">
                    {OUTCOME_LABELS[call.outcome as CallOutcome]}
                  </Text>
                </HStack>
              </Badge>
            </HStack>
